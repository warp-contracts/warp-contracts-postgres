...
describe('Postgres sort key cache transactions testing', () => {
  it('access range keys during active transaction and commit', async () => {
    ...
    const kvKeys = Array.from(
      (
        await sut.kvMap(getSortKey(sortKey), { gte: "key.", lt: "key.\xff" })
      ).keys()
    );
  ...
  it("keys order natural and reversed", async () => {
    ...
    const naturalOrder = Array.from(
      (
        await sut.kvMap(getSortKey(sortKey), { gte: "user.11", lt: "user.14" })
      ).keys()
    );
