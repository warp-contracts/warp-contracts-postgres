...
describe('Postgres cache', () => {
  ...
  it('should keep the latest insert, even it is the smallest one', async () => {
    ...
    const result5 = await sut.get(new CacheKey(getContractId(0), getSortKey(5)));
    const result7 = await sut.get(new CacheKey(getContractId(0), getSortKey(7)));
    const result2 = await sut.get(new CacheKey(getContractId(0), getSortKey(2)));
